...
If `control_assess` is unavailable, fails, or returns empty signals / `provider_error:*` (including disabled or cooldown assessments), perform the same rubric manually and label the result **unassessed by a Decision Provider**. A fail-open `CONTINUE` does not prove recovery; do not repeatedly retry the provider instead of investigating. Do not call the same failed tool again without a changed hypothesis.
